...
def run_sam_pipeline(args):

    ROOT_DIR = "./data/TestDataset"
    SAVE_DIR = "./sam_predictions"

    model = LangSAM()

    for dataset in tqdm.tqdm(os.listdir(ROOT_DIR)):
        img_folder = os.path.join(ROOT_DIR, dataset, "Imgs")
        os.makedirs(os.path.join(SAVE_DIR, dataset), exist_ok=True)
        for img_name in tqdm.tqdm(os.listdir(img_folder)):
            image_pil = Image.open(os.path.join(img_folder, img_name)).convert("RGB")
            text_prompt = "animal, insect, fish or human"
            try:
            ...
            cv2.imwrite(os.path.join(SAVE_DIR, dataset, img_name), masks)
